...
if($_POST){   
  ...
  if(camposVaciosTesis($nombreP,$descripcion)){
    ...
    if($id_Tesis>0){
      ...
      if($resulta){
        $sql = "UPDATE tesis SET nombre='$nombreP', descripcion='$descripcion', url='$archivo' WHERE idE='$Id_Usuario'";
      ...
      $resulta=@move_uploaded_file($_FILES["file"]["tmp_name"], $archivo);
      if($resulta){
        $sql = "INSERT INTO tesis (idE, nombre, descripcion, url) VALUES ('$Id_Usuario','$nombreP','$descripcion', '$archivo')";
        $resultado=$mysqli->query($sql);
