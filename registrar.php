      ...
      if($rol=="Jurado"){
        if(camposVaciosDoc($nombres, $apellidos, $email, $cedula, $codigo, $celular, $rol, $username, $password)){
                ...
                if(usuarioExiste($username)){
                  ...
                  $tipoUser = 3;
                  $sql = "INSERT INTO docentes (cedula, username, password, nombres, apellidos, correo, celular, codigo, rol, tipoUser) VALUES ('$cedula','$username','$password','$nombres','$apellidos','$email',   '$celular','$codigo','JURADO',3)";
                  $resultado=$mysqli->query($sql);
                  if($resultado){
                    $mensaje ="Se registró al $rol $nombres $apellidos exitosamente ";
                  }
